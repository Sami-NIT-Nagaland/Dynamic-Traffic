...
speeds = {'car':2.5, 'bus':5.8, 'truck':2.4, 'bike':5.5}  # average speeds of vehicles
...
class TrafficSignal:
    def __init__(self, red, yellow, green):
        self.red = red
        self.yellow = yellow  # Corrected this line
        self.green = green
        self.signalText = ""
...
def calculateGreenTime(lane):
    # Count the vehicles that haven't crossed the stop line
    vehicle_count = sum(
        1 for i in range(3)
        for vehicle in vehicles[directionNumbers[lane]][i]
        if vehicle.crossed == 0
    )
    
    # If no vehicles, return 5 seconds (minimum green light duration)
    if vehicle_count == 0:
        return 5  # minimum green light time when no vehicles
    
    # Calculate the green light time based on the number of vehicles
    green_time = vehicle_count * 2  # 2 seconds per vehicle
    
    # Limit the green time to a maximum of 20 seconds
    if green_time > 20:
        green_time = 20
...
def repeat():
    ...
    green_time = calculateGreenTime(currentGreen)
    
    # Ensure the green light is held for at least 5 seconds
    if green_time < 5:
        green_time = 5
    ...
    # If no vehicles and we held green for the minimum time, move to the next
    if green_time == 5 and calculateGreenTime(currentGreen) == 0:
        signals[currentGreen].green = 0  # Skip the green light since there are no vehicles
    else:
        signals[currentGreen].green = green_time
    
    # Hold the green light for the calculated time
    ...
    # Hold the yellow light for its duration
    while signals[currentGreen].yellow > 0:
        updateValues()
        time.sleep(1)
    
    currentYellow = 0  # Turn off the yellow light
...
def generateVehicles():
    while(True):
        ...
        # Increase sleep time to reduce the frequency of vehicle generation
        time.sleep(3)  # Adjusted to 3 seconds for fewer vehicles
...
class Main:
    ...
    pygame.display.set_caption("HyderbadPOLICE")
